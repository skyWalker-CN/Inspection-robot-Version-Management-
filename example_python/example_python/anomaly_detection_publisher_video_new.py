...
class AnomalyDetectionPublisher(Node):
    ...
    # 初始化RTSP视频流（替换原来的ROS话题订阅）
        ...
        # 定时器
        self.timer = self.create_timer(0.033, self.timer_callback)  # 约30Hz
...
# 新增RTSP流初始化方法
    def init_rtsp_stream(self):
         """初始化RTSP视频流"""
         
         try:
             if not self.rtsp_url:
                 self.get_logger().error("未提供RTSP URL，无法初始化视频流")
                 return
            
             self.cap = cv2.VideoCapture(self.rtsp_url)
        
        # 设置OpenCV参数以提高RTSP流稳定性
             self.cap.set(cv2.CAP_PROP_BUFFERSIZE, 1)
             self.cap.set(cv2.CAP_PROP_FPS, 30)
             self.cap.set(cv2.CAP_PROP_FOURCC, cv2.VideoWriter_fourcc(*'H264'))
        
             if not self.cap.isOpened():
                 self.get_logger().error(f"无法打开RTSP视频流: {self.rtsp_url}")
                 return
        
        # 启动RTSP流读取线程
             self.rtsp_thread = threading.Thread(target=self.rtsp_stream_worker, daemon=True)
             self.rtsp_thread.start()
             self.get_logger().info(f"✅ RTSP视频流已启动: {self.rtsp_url}")
        
         except Exception as e:
             self.get_logger().error(f"初始化RTSP视频流失败: {e}")
    # 新增RTSP流工作线程
    def rtsp_stream_worker(self):
        reconnect_attempts = 0
        max_reconnect_attempts = 5
        reconnect_delay = 3  # 重连延迟（秒）
    
        while self.rtsp_running and rclpy.ok():
            try:
                ret, frame = self.cap.read()
            
                if not ret:
                    self.get_logger().warning("RTSP流读取失败，尝试重连...")
                    reconnect_attempts += 1
                
                    if reconnect_attempts > max_reconnect_attempts:
                        self.get_logger().error("RTSP流重连次数超过限制，退出")
                        break
                ...
                # 尝试重新连接
                    ...
                    if not self.cap.isOpened():
                        self.get_logger().warning(f"RTSP流重连失败，尝试 {reconnect_attempts}/{max_reconnect_attempts}")
                        continue
                
                    self.get_logger().info("RTSP流重连成功")
                    reconnect_attempts = 0
                    continue
            ...
            # 控制帧率，避免处理过快
                ...
                self.get_logger().error(f"RTSP流处理异常: {e}")
    ...
    def timer_callback(self):
        ...
        # 显示实时画面
            if self.show_display:
                cv2.imshow('Anomaly Detection - RTSP Live Feed', display_frame)
                cv2.waitKey(1)
        ...
        # 发布异常数据
            ...
            self.get_logger().error(f"帧处理失败: {e}")
...
def parse_args():
    ...
    parser.add_argument('--rtsp-url', type=str, default = 'rtsp://172.20.10.5:554/stream1', help='RTSP视频流URL')
