...
class AsyncNavigator(Node):
    def __init__(self):
        ...
        # ---------- 充电相关 ----------
        self.going_to_charge = False            # 是否处于充电中断流程
        self.pending_charge = False             # 取消目标后是否需要插入充电路径
        self.charge_point = (10.6, 0.86)        # 充电桩坐标（x, y）
    ...
    def _navigate_next(self):
        if self.current_idx >= len(self.current_waypoints):
            ...
            # 充电桩路径结束：停止导航，等待充电完成
            if self._is_charge_path():
                self.get_logger().info("充电桩路径执行完毕，等待充电完成...")
    ...
    def _is_charge_path(self):
        """判断当前路径是否为充电桩路径（单点且坐标匹配）"""
        if len(self.current_waypoints) == 1:
            cp = self.current_waypoints[0].pose
            return (abs(cp.position.x - self.charge_point[0]) < 0.001 and
                    abs(cp.position.y - self.charge_point[1]) < 0.001)
        return False
    ...
    def _result_callback(self, future):
        ...
        # 更新进度：仅在执行用户路径且成功时推进
        if not self._is_origin_path() and not self._is_charge_path() and success:
    ...
    # ---------- 充电相关辅助 ----------
    def _insert_charge_path(self):
        """构建充电桩路径并插入队列（单点）"""
        charge_pose = PoseStamped()
        charge_pose.header.frame_id = 'map'
        charge_pose.header.stamp = self.get_clock().now().to_msg()
        charge_pose.pose.position.x = self.charge_point[0]
        charge_pose.pose.position.y = self.charge_point[1]
        charge_pose.pose.position.z = 0.0
        charge_pose.pose.orientation.w = 1.0
        self.path_queue.insert(0, [charge_pose])
        self.get_logger().info(f"已插入充电桩路径: ({self.charge_point[0]}, {self.charge_point[1]})")
    ...
    def _status_callback(self, msg):
        ...
        # 状态 -> 2：需要充电，中断当前任务并前往充电桩
        elif new_status == "2" and old_status != "2":
            self.get_logger().info("收到充电指令，中断当前任务前往充电桩")
            ...
            if self.goal_handle is not None:
                ...
                # 没有正在执行的目标，直接插入充电路径并启动
                self.path_queue.clear()
                self._insert_charge_path()
                if not self.navigating:
                    self._start_next_path()

        # 状态 2 -> 0：充电完成，恢复之前的用户任务
        elif old_status == "2" and new_status == "0":
            self.get_logger().info("充电完成，状态恢复正常")
