...
class AnomalyDetectionPublisher(Node):
    def __init__(self, args):
        ...
        self.conf_threshold = args.conf  # 保留通用阈值用于向后兼容
        
        # 分别设置各类目标的置信度阈值
        self.fire_conf = args.fire_conf
        self.smoke_conf = args.smoke_conf
        self.face_conf = args.face_conf
        self.plate_conf = args.plate_conf
        ...
        # 异常检测相关
        self.models = {}
        self.last_screenshot_time = {}  # 记录上次截图时间，防止重复截图
        self.screenshot_interval = 10  # 截图间隔（秒）- 调整为10秒便于测试
        
        # 异常目标跟踪器
        self.target_tracker = AnomalyTargetTracker(disappear_timeout=args.target_timeout)
        
        # 火焰和烟雾检测冷却时间控制
        self.fire_smoke_cooldown = 20.0  # 20秒冷却时间
        ...
        self.ocr_skip_frames = 1  # 提高识别频率：每帧尝试一次OCR
        # 低开销日志控制与快速同步识别帧标记
        ...
        # 创建必要的文件夹
        # 获取当前脚本所在目录的父目录（anomaly_detect包目录）
        current_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        
        # 使用绝对路径确保截图文件夹在工作空间根目录
        self.history_folder = os.path.join("/home/jetson/ros2_ws", "history_anomaly")
        self.known_faces_folder = self.find_known_faces_folder()
        ...
        try:
            ...
            print(f"✅ 截图文件夹已创建: {self.history_folder}")
        except Exception as e:
            print(f"❌ 创建截图文件夹失败: {e}")
            # 如果无法创建在工作空间，则使用当前目录
            self.history_folder = "history_anomaly"
            os.makedirs(self.history_folder, exist_ok=True)
            print(f"⚠️  使用备用截图路径: {os.path.abspath(self.history_folder)}")
        
        # known_faces文件夹已存在，不需要创建
        ...
        print(f"🎯 置信度阈值设置:")
        print(f"   - 火焰检测: {self.fire_conf}")
        print(f"   - 烟雾检测: {self.smoke_conf}")
        print(f"   - 人脸检测: {self.face_conf}")
        print(f"   - 车牌检测: {self.plate_conf}")
        print(f"🎯 目标跟踪器已启动 (消失超时: {self.target_tracker.disappear_timeout}秒)")
        print(f"⏰ 火焰和烟雾警报冷却时间: {self.fire_smoke_cooldown}秒")
        print("📡 发布话题:")
    ...
    def find_known_faces_folder(self):
        """查找已知人脸文件夹，优先在ROS2包目录中查找"""
        # 获取当前脚本所在目录的父目录（anomaly_detect包目录）
        current_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        
        possible_paths = [
            os.path.join(current_dir, "data", "known_faces"),  # 相对于包的数据目录
            "/home/jetson/ros2_ws/src/anomaly_detect/data/known_faces",  # 绝对路径备用
            "/home/jetson/ros2_ws/install/anomaly_detect/lib/python3.10/site-packages/anomaly_detect/data/known_faces",  # 安装后的路径
            "/home/jetson/ros2_ws/install/anomaly_detect/lib/python3.10/site-packages/data/known_faces",  # 错误路径（用于兼容）
            "data/known_faces",  # 相对路径
        ]
        
        for path in possible_paths:
            if os.path.exists(path) and os.path.isdir(path):
                print(f"✅ 找到已知人脸文件夹: {path}")
                return path
        
        # 如果都找不到，返回默认路径并给出警告
        print("⚠️  未找到已知人脸文件夹，将使用默认路径")
        return os.path.join(current_dir, "data", "known_faces")
    ...
    def find_whitelist_file(self):
        ...
        possible_paths = [
            os.path.join(current_dir, "data", "whitelist", "whitelist.txt"),  # 相对于包的数据目录
            "/home/jetson/ros2_ws/src/anomaly_detect/data/whitelist/whitelist.txt",  # 绝对路径备用
            "data/whitelist/whitelist.txt",  # 相对路径
            "whitelist.txt"  # 当前目录
        ]
    ...
    def find_dlib_model_paths(self):
        ...
        # 可能的dlib模型路径
        possible_base_paths = [
            os.path.join(current_dir, "models"),  # 相对于包的模型目录
            "/home/jetson/ros2_ws/src/anomaly_detect/models",  # 绝对路径备用
            "models"  # 相对路径
        ]
    ...
    def detect_plate_optimized(self, frame):
        ...
        while not self.ocr_result_queue.empty():
            try:
                ...
                if result_frame_idx >= self.ocr_frame_counter - 8:  # 缩小窗口，减少跨帧等待
        ...
        if 'plate' in self.models:
            results = self.models['plate'](frame, conf=self.plate_conf)
            for result in results:
                boxes = result.boxes
    ...
    def init_camera(self):
        ...
        try:
            # 检查是否为数字设备ID（USB摄像头）
            if self.camera_source.isnumeric():
                device_id = int(self.camera_source)
                print(f"🔍 尝试打开USB摄像头: {device_id}")
    ...
    def detect_anomalies(self, frame):
        ...
        try:
            ...
            if 'face' in self.models:
                results = self.models['face'](frame, conf=self.face_conf)
                for result in results:
                    boxes = result.boxes
                    if boxes is not None:
                        for box in boxes:
    ...
    def is_valid_plate(self, text):
        ...
        if 'I' in t or 'O' in t:
            return False
        # 要求至少包含3个数字，避免纯字母串
        if sum(ch.isdigit() for ch in t[2:]) < 3:
            return False
        return True
    ...
    def save_anomaly_screenshot(self, frame, anomaly_type):
        ...
        # 检查是否在截图间隔内 - 只对相同类型的异常进行间隔控制
        if anomaly_type in self.last_screenshot_time:
            time_since_last = current_time - self.last_screenshot_time[anomaly_type]
            if time_since_last < self.screenshot_interval:
                print(f"⏰ {anomaly_type} 截图间隔未到，跳过截图 (剩余: {self.screenshot_interval - time_since_last:.1f}秒)")
                # 返回最近的截图路径（如果存在）
                recent_files = []
                try:
                    for file in os.listdir(self.history_folder):
                        if file.startswith(f"{anomaly_type}_") and file.endswith('.jpg'):
                            file_path = os.path.join(self.history_folder, file)
                            file_time = os.path.getmtime(file_path)
                            if current_time - file_time < self.screenshot_interval * 2:  # 2倍间隔内的文件
                                recent_files.append((file_path, file_time))
                    
                    if recent_files:
                        # 返回最新的截图
                        recent_files.sort(key=lambda x: x[1], reverse=True)
                        recent_path = recent_files[0][0]
                        print(f"🔄 使用最近的截图: {recent_path}")
                        return recent_path
                except Exception as e:
                    print(f"⚠️  查找最近截图失败: {e}")
        ...
        try:
            ...
            if success and os.path.exists(filepath):
                ...
                print(f"❌ 截图保存失败: cv2.imwrite返回{success}, 文件存在:{os.path.exists(filepath) if success else 'N/A'}")
                return None
        except Exception as e:
            print(f"❌ 截图保存异常: {e}")
            return None
    ...
    def timer_callback(self):
        ...
        try:
            ...
            # 保存异常截图 - 简化逻辑，每个异常类型都尝试保存截图
            for anomaly in anomalies:
                # 直接使用异常类型作为截图标识
                screenshot_path = self.save_anomaly_screenshot(frame, anomaly['type'])
                if screenshot_path and os.path.exists(screenshot_path):
                    anomaly['image_path'] = screenshot_path
                    print(f"✅ 异常截图已保存并关联: {anomaly['type']} -> {screenshot_path}")
                else:
                    # 如果截图保存失败，尝试强制保存一次
                    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S_%f")[:-3]
                    filename = f"{anomaly['type']}_force_{timestamp}.jpg"
                    filepath = os.path.join(self.history_folder, filename)
                    try:
                        os.makedirs(self.history_folder, exist_ok=True)
                        success = cv2.imwrite(filepath, frame)
                        if success and os.path.exists(filepath):
                            anomaly['image_path'] = filepath
                            print(f"✅ 强制保存异常截图成功: {filepath}")
                        else:
                            anomaly['image_path'] = ''
                            print(f"❌ 强制保存异常截图失败: {filepath}")
                    except Exception as e:
                        anomaly['image_path'] = ''
                        print(f"❌ 强制保存异常截图异常: {e}")
            ...
            for anomaly in anomalies:
                ...
                if anomaly_type == 'fire':
                    if current_time - self.last_fire_alert_time < self.fire_smoke_cooldown:
                        ...
                        remaining_time = self.fire_smoke_cooldown - (current_time - self.last_fire_alert_time)
                        print(f"🔥 火焰警报冷却中，剩余时间: {remaining_time:.1f}秒")
                    else:
                        self.last_fire_alert_time = current_time
                        print(f"🔥 火焰警报冷却时间已过，发布新警报")
                elif anomaly_type == 'smoke':
                    if current_time - self.last_smoke_alert_time < self.fire_smoke_cooldown:
                        should_publish = False
                        remaining_time = self.fire_smoke_cooldown - (current_time - self.last_smoke_alert_time)
                        print(f"💨 烟雾警报冷却中，剩余时间: {remaining_time:.1f}秒")
                    else:
                        self.last_smoke_alert_time = current_time
                        print(f"💨 烟雾警报冷却时间已过，发布新警报")
                elif anomaly_type == 'unknown_person':
                    if current_time - self.last_face_alert_time < self.face_cooldown:
                        should_publish = False
                        remaining_time = self.face_cooldown - (current_time - self.last_face_alert_time)
                        print(f"👤 人脸警报冷却中，剩余时间: {remaining_time:.1f}秒")
                    else:
                        self.last_face_alert_time = current_time
                        print(f"👤 人脸警报冷却时间已过，发布新警报")
...
def parse_args():
    ...
    parser.add_argument('--source', type=str, default='0', help='相机源')
    parser.add_argument('--fire-model', type=str, 
                       default='/home/jetson/ros2_ws/src/anomaly_detect/models/fire_smoke_best.pt', 
                       help='火焰烟雾模型路径')
    parser.add_argument('--plate-model', type=str, 
                       default='/home/jetson/ros2_ws/src/anomaly_detect/models/plate_best.pt', 
                       help='车牌模型路径')
    parser.add_argument('--face-model', type=str, 
                       default='/home/jetson/ros2_ws/src/anomaly_detect/models/yolov8n-face.pt', 
                       help='人脸模型路径')
    ...
    parser.add_argument('--smoke-conf', type=float, default=0.5, help='烟雾检测置信度阈值')
    parser.add_argument('--face-conf', type=float, default=0.6, help='人脸检测置信度阈值')
    parser.add_argument('--plate-conf', type=float, default=0.8, help='车牌检测置信度阈值')
