...
class AsyncNavigator(BasicNavigator):
    ...
    def _task_wait(self, params):
        ...
        if not math.isfinite(seconds) or seconds <= 0:
            self.get_logger().error(f"[wait] 等待时间无效: {seconds}")
            return False
    ...
    def _task_spin(self, params):
        ...
        if not math.isfinite(angle_deg):
            self.get_logger().error(f"[spin] 角度参数无效: {angle_deg}")
            return False
        ...
        if not math.isfinite(duration) or duration <= 0:
            self.get_logger().error(f"[spin] 计算出的持续时间无效: {duration}")
            return False
        if duration > 120.0:  # 防止异常长时间旋转
            self.get_logger().warn(f"[spin] 持续时间过长: {duration:.2f}s，请检查参数")
            # 可以根据需要决定是否直接返回 False，这里选择继续执行但发出警告

        self.get_logger().info(
            f"[spin] 原地旋转 {angle_deg}°，角速度 {angular_z} rad/s，持续 {duration:.2f} 秒"
        )
    ...
    def _task_cmd_vel(self, params):
        ...
        if not math.isfinite(linear_x) or not math.isfinite(angular_z) or not math.isfinite(duration):
            self.get_logger().error(f"[cmd_vel] 参数包含无效数值: {params}")
            return False
        if duration <= 0:
            self.get_logger().error(f"[cmd_vel] 持续时间无效: {duration}")
            return False
        if duration > 120.0:
            self.get_logger().warn(f"[cmd_vel] 持续时间过长: {duration:.2f}s，请检查参数")
    ...
    def _publish_twist(self, linear_x=0.0, angular_z=0.0, duration=1.0):
        """
        按固定频率发布速度指令，持续指定时长，结束后补发停止指令。
        使用基于绝对时间的循环，确保即使 duration 很短也能至少发送一次，
        且不会因为 period 大于 duration 而提前退出。
        """
        # 有效性检查
        if not math.isfinite(duration) or duration <= 0:
            self.get_logger().error(f"[publish_twist] 无效的持续时间: {duration}")
            return
        ...
        rate_hz = max(1.0, float(self.cmd_vel_rate_hz))
        period = 1.0 / rate_hz
        end_time = time.time() + duration

        self.get_logger().info(
            f"[publish_twist] 开始发布速度: vx={linear_x}, wz={angular_z}, "
            f"duration={duration:.3f}s, rate={rate_hz}Hz, period={period:.3f}s"
        )
        ...
        while rclpy.ok() and time.time() < end_time:
            ...
            # 计算剩余时间，避免最后一次 sleep 超过结束时间
            remaining = end_time - time.time()
            if remaining <= 0:
                break
            # 睡眠时间不超过剩余时间，也不超过一个周期
            time.sleep(min(period, remaining))

        # 确保停止
        self._stop()
        self.get_logger().info(f"[publish_twist] 发布结束，共发送 {count} 条速度指令")
