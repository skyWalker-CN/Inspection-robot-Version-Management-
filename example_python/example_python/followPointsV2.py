...
class AsyncNavigator(BasicNavigator):
    ...
    def _follow_waypoints_thread(self, waypoints):
        """后台线程：逐个尝试路径点，失败则跳过；若末尾点失败，则返回原点"""
        try:
            current_waypoints = waypoints
            last_result = None  # 记录最后一个路径点的执行结果
            ...
            # 所有路径点（含队列中的）都执行完毕，若最后一个尝试的点失败，则返回原点
            if last_result is not None and last_result == TaskResult.FAILED:
                self.get_logger().warn("剩余目标点都无法到达，尝试返回原点")
    ...
    def _return_to_origin(self):
        ...
        if result == TaskResult.SUCCEEDED:
            ...
            self.get_logger().error(f"返回原点失败: {result}，尝试人工干预")
