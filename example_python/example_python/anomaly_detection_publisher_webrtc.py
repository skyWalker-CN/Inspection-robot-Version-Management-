#!/usr/bin/env python3
"""
智能巡检机器人异常检测发布节点（WebRTC 图像流版）
集成火焰、烟雾、人脸、车牌检测功能
支持实时画面显示、异常数据发布和截图保存
通过 WebRTC 将处理后的图像推送到浏览器
"""
...
try:
    ...
    print(f"⚠️  WebRTC 库导入失败: {e}")
    print("请安装: pip install aiortc av websockets")
...
class ProcessedFrameTrack(VideoStreamTrack):
    """
    WebRTC 视频轨道，从节点的 latest_frame 获取最新处理后的帧并发送。
    """
...
class AnomalyDetectionPublisher(Node):
    def __init__(self, args):
        ...
        self.show_display = not args.no_display  # 根据参数控制实时画面
        ...
        self.use_video_file = bool(self.video_path)
        ...
        if self.use_video_file:
            self.init_video_source(args.video_path)
        else:
        ...
        # ---------- WebRTC 相关初始化 ----------
        self.frame_condition = threading.Condition()   # 用于线程同步
        self.latest_frame = None                       # 最新处理后的帧（BGR）
        ...
        print("🌐 WebRTC 服务器线程已启动 (ws://0.0.0.0:8080)")

    # ---------- 帧获取方法供 WebRTC 轨道调用 ----------
    def get_latest_frame(self):
        """供 WebRTC 轨道调用的阻塞方法：等待并返回最新处理后的帧（副本）"""
    ...
    # ---------- 视频源处理方法 ----------
    def init_video_source(self, video_path):
        """初始化本地视频源，并使用定时器不断读取帧"""
        if not video_path:
            self.get_logger().error("未提供视频文件路径，无法初始化视频源")
            return
        ...
        if fps <= 0 or np.isnan(fps):
            fps = 25.0
        fps = max(min(fps, 60.0), 1.0)  # 限制在1-60 FPS之间，避免过快或过慢
        interval = 1.0 / fps

        if self.frame_timer is not None:
            self.frame_timer.cancel()

        self.frame_timer = self.create_timer(interval, self.video_frame_callback)
        self.get_logger().info(
            f"✅ 成功加载视频文件: {video_path}, FPS≈{fps:.2f}, 定时器间隔 {interval:.3f}s"
        )
    ...
    def video_frame_callback(self):
        ...
        if not ret:
            if self.video_loop:
                self.get_logger().info("视频到达末尾，回到开头重新播放")
                self.cap.set(cv2.CAP_PROP_POS_FRAMES, 0)
                ret, frame = self.cap.read()
                if not ret:
                    self.get_logger().error("循环播放失败，无法重新读取帧")
                    self.stop_video_stream()
                    return
            else:
                self.get_logger().info("视频已播放完毕，停止读取")
                self.stop_video_stream()
                return

        # 直接处理帧，process_frame 内部会设置 self.latest_frame
    ...
    def image_callback(self, msg):
        ...
        try:
            ...
            # 直接处理图像，process_frame 内部会设置 self.latest_frame
    ...
    def draw_chinese_text(self, img, text, position, font_size=20, color=(255, 255, 255), bg_color=None):
        ...
        try:
            ...
            cv2.putText(img, text, position, cv2.FONT_HERSHEY_SIMPLEX, 1.2, color, 3)
            return img
    ...
    def run_webrtc_server(self):
        ...
        try:
            ...
            async def start_server():
                ...
                # 保持服务器运行（不阻塞，因为 run_forever 会保持循环）

            # 运行 start_server 协程直到服务器启动完成
    ...
    def create_ice_candidate(self, cand):
        ...
        # 返回 SimpleNamespace 对象（aiortc 可接受）
    ...
    def cleanup(self):
        ...
        # 停止 WebRTC 服务器
        if hasattr(self, 'websocket_server') and self.websocket_server is not None:
            # 在事件循环线程中关闭服务器
            async def close_server():
                self.websocket_server.close()
                await self.websocket_server.wait_closed()
            if self.webrtc_loop is not None:
                # 使用 run_coroutine_threadsafe 从主线程向事件循环线程提交关闭任务
                future = asyncio.run_coroutine_threadsafe(close_server(), self.webrtc_loop)
                try:
                    future.result(timeout=2)  # 等待最多2秒
                except Exception as e:
                    print(f"关闭 WebSocket 服务器时出错: {e}")
        ...
        if self.webrtc_thread.is_alive():
            self.webrtc_thread.join(timeout=2)
        print("✅ WebRTC 服务器线程已停止")


def parse_args():
    parser = argparse.ArgumentParser(description="智能巡检机器人异常检测发布节点 (WebRTC 图像流)")
    parser.add_argument('--fire-model', type=str, default='/home/jetson/ros2_ws/src/example_python/models/best.pt', help='火焰烟雾模型路径')
    ...
    parser.add_argument('--video-path', type=str, default='', help='本地视频文件路径（留空则订阅相机话题）')
    parser.add_argument('--video-loop', action='store_true', help='视频播放结束后循环播放')
