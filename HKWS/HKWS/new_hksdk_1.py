...
# -*- coding: utf-8 -*-
"""
ROS2节点：海康SDK云台控制 + 实时预览 + WebRTC（无显示窗口，持续发布图像）
...
def resize_frame(frame, target_width=OUTPUT_WIDTH, target_height=OUTPUT_HEIGHT):
    """将图像缩放至目标分辨率"""
    if frame is None:
        return None
    ...
    async def recv(self):
        ...
        try:
            ...
            if self._last_frame is not None:
                ...
                # 缺省帧也使用缩放分辨率
                frame_bgr = np.zeros((OUTPUT_HEIGHT, OUTPUT_WIDTH, 3), dtype=np.uint8)
...
class HikCameraNode(Node):
    def __init__(self):
        ...
        self.preview_handle = start_preview(self.user_id, CHANNEL, 1)
        ...
        self.get_logger().info("节点已启动，持续发布640×360图像到 /hik_camera/image_raw")
