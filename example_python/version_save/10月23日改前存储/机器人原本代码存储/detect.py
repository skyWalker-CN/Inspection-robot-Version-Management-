...
sys.path.append('/home/robot/anaconda3/envs/detection/lib/python3.10/site-packages/')
...
class AnomalyDetectionPublisher(Node):
    def __init__(self, args):
        ...
        # 异常检测相关
        self.models = {}
        self.last_screenshot_time = {}  # 记录上次截图时间，防止1分钟内重复截图
        self.screenshot_interval = 60  # 截图间隔（秒）
        ...
        self.ocr_skip_frames = 2  # 每2帧执行一次OCR
        ...
        print("🔥 异常检测发布节点已启动")
        print(f"📷 相机源: {args.source}")
        print(f"🎯 置信度阈值: {args.conf}")
        print(f"🎯 目标跟踪器已启动 (消失超时: {self.target_tracker.disappear_timeout}秒)")
        print("📡 发布话题:")
        print("   - /web_image/compressed (实时画面)")
        print("   - /anomaly_detection/data (异常数据)")
        print("   - /anomaly_detection/alerts (警报，已过滤重复)")
        print(f"📁 异常截图保存路径: {self.history_folder}")

    def find_whitelist_file(self):
        """查找白名单文件，优先在ROS2包目录中查找"""
        possible_paths = [
            "/home/robot/ros2_ws/src/example_python/data/whitelist/whitelist.txt",
        ]
    ...
    def find_dlib_model_paths(self):
        ...
        # 可能的dlib模型路径
        possible_base_paths = [
            "/home/robot/ros2_ws/install/example_python/share/example_python/models",
            "/home/robot/ros2_ws/src/example_python/models",
            "models"
        ]
    ...
    def init_detection_models(self):
        ...
        try:
            ...
            # 初始化Tesseract-OCR（用于车牌识别）
            try:
                # 测试Tesseract是否可用
                version = pytesseract.get_tesseract_version()
                print(f"✅ Tesseract版本: {version}")
                
                # 配置多种Tesseract参数用于不同情况
                self.tesseract_configs = [
                    r'--oem 3 --psm 7 -c tessedit_char_whitelist=0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ京津沪渝冀豫云辽黑湘皖鲁新苏浙赣鄂桂甘晋蒙陕吉闽贵粤青藏川宁琼使领',
                    r'--oem 3 --psm 8 -c tessedit_char_whitelist=0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ京津沪渝冀豫云辽黑湘皖鲁新苏浙赣鄂桂甘晋蒙陕吉闽贵粤青藏川宁琼使领',
                    r'--oem 3 --psm 6 -c tessedit_char_whitelist=0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ京津沪渝冀豫云辽黑湘皖鲁新苏浙赣鄂桂甘晋蒙陕吉闽贵粤青藏川宁琼使领',
                    r'--oem 3 --psm 13 -c tessedit_char_whitelist=0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ京津沪渝冀豫云辽黑湘皖鲁新苏浙赣鄂桂甘晋蒙陕吉闽贵粤青藏川宁琼使领'
                ]
                self.tesseract_config = self.tesseract_configs[0]  # 默认使用第一个
                print("✅ Tesseract-OCR初始化成功")
            except Exception as e:
                print(f"❌ Tesseract-OCR初始化失败: {e}")
                print("请确保已安装tesseract-ocr和pytesseract")
                self.tesseract_configs = [r'--oem 3 --psm 8', r'--oem 3 --psm 7', r'--oem 3 --psm 6']
    ...
    def setup_chinese_font(self):
        ...
        try:
            ...
            font_paths = [
                ...
                "/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf",  # DejaVu Sans (英文)
            ...
            for font_path in font_paths:
                try:
                    ...
                    print(f"⚠️  字体文件存在但加载失败: {font_path} - {font_error}")
                    continue
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    text_width, text_height = 100, 20  # 默认尺寸
            ...
            try:
                ...
                print(f"PIL文本绘制失败: {e}")
                # 转换回OpenCV格式后使用OpenCV绘制
                img_cv = cv2.cvtColor(np.array(img_pil), cv2.COLOR_RGB2BGR)
                cv2.putText(img_cv, text, position, cv2.FONT_HERSHEY_SIMPLEX, 0.6, color, 2)
                return img_cv
    ...
    def detect_plate_optimized(self, frame):
        ...
        while not self.ocr_result_queue.empty():
            try:
                ...
                # 增加时间窗口到20帧，确保OCR结果不会因为处理时间长而被丢弃
                if result_frame_idx >= self.ocr_frame_counter - 20:  
        ...
        if 'plate' in self.models:
            results = self.models['plate'](frame, conf=self.conf_threshold)
            for result in results:
                boxes = result.boxes
                if boxes is not None:
                    for box in boxes:
                        ...
                        if cached_result:
                            ...
                            plate_info['plate_text'] = plate_number
                            # 修复：使用清理后的文本进行白名单比较
                            clean_plate_number = self.clean_plate_text(plate_number)
                            plate_info['is_registered'] = clean_plate_number in self.whitelist
                            plate_info['ocr_processed'] = True
                            # 调试信息
                            if clean_plate_number != plate_number:
                                print(f"🔍 车牌文本清理: 原始='{plate_number}' 清理后='{clean_plate_number}' 是否在白名单={'✅' if plate_info['is_registered'] else '❌'}")
                            # 添加更详细的调试信息
                            if plate_info['is_registered']:
                                print(f"✅ 车牌匹配成功: '{clean_plate_number}' 在白名单中")
                            else:
                                print(f"❌ 车牌匹配失败: '{clean_plate_number}' 不在白名单中")
                                print(f"   当前白名单: {sorted(list(self.whitelist))}")
    ...
    def preprocess_plate_image(self, plate_img):
        """预处理车牌图像以提高OCR精度（兼容函数，调用Tesseract专用预处理）"""
        return self.preprocess_plate_image_for_tesseract(plate_img)
    ...
    def init_camera(self):
        """初始化相机"""
        try:
            source = int(self.camera_source) if self.camera_source.isnumeric() else self.camera_source
            print(f"🔍 尝试打开相机设备: {source}")
    ...
    def detect_anomalies(self, frame):
        ...
        try:
            ...
            if 'fire' in self.models:
                results = self.models['fire'](frame, conf=self.conf_threshold)
                for result in results:
                    boxes = result.boxes
                    if boxes is not None:
                        for box in boxes:
            ...
            if 'face' in self.models:
                results = self.models['face'](frame, conf=self.conf_threshold)
                for result in results:
                    boxes = result.boxes
                    if boxes is not None:
                        for box in boxes:
            ...
            for plate_info in plate_detections:
                ...
                if (plate_info['ocr_processed'] and 
                    ...
                    # 清理车牌文本
                    clean_plate_text = self.clean_plate_text(plate_info['plate_text'])
                    
                    # 验证车牌格式（可选，如果有车牌格式验证函数）
                    anomaly = {
    ...
    def clean_plate_text(self, text):
        ...
        print(f"🔍 清理前车牌文本: '{text}' (长度: {len(text)})")
        ...
        for i, char in enumerate(text):
            ...
            if i == 0:
                ...
                if '\u4e00' <= char <= '\u9fa5':
                    ...
                    result += province_map.get(char, char)
            elif i == 1:
                # 第二位应该是字母
                if char.isalpha():
                    result += char
                ...
                # 后面的位置可以是字母或数字
                if char in replacements and (i >= 2):  # 只对第3位及以后应用数字替换
                    result += replacements[char]
                else:
                    result += char
        
        # 移除所有非中文、大写字母和数字的字符
        cleaned = re.sub(r'[^\u4e00-\u9fa5A-Z0-9]', '', result)
        
        # 验证车牌格式
        if len(cleaned) >= 7:
            # 标准格式：1个汉字 + 1个字母 + 5个字母或数字
            if ('\u4e00' <= cleaned[0] <= '\u9fa5' and 
                cleaned[1].isalpha() and 
                len(cleaned) <= 8):  # 允许7-8位
                print(f"✅ 清理后车牌文本: '{cleaned}' (标准格式)")
                return cleaned
        
        # 如果不符合标准格式，但长度合理，仍然返回
        if 4 <= len(cleaned) <= 10:
            print(f"⚠️ 清理后车牌文本: '{cleaned}' (非标准格式但可接受)")
            return cleaned
        
        print(f"❌ 清理后车牌文本: '{cleaned}' (格式不符合要求)")
        return cleaned  # 即使格式不对也返回，让上层判断
        ...
        for anomaly in anomalies:
            ...
            if len(bbox) == 4:
                ...
                colors = {
                    'fire': (0, 0, 255),      # 红色
                    'smoke': (0, 165, 255),   # 橙色
                    'unknown_person': (255, 0, 0),    # 蓝色
                    'unknown_vehicle': (0, 255, 255)  # 黄色
                }
                ...
                if len(bbox) == 4:
                    ...
                    if detection['type'] == 'known_person':
                        ...
                        if ocr_processed and is_registered and plate_text not in ["size_error", "no_result", "low_confidence", "ocr_error"]:
                            ...
                            clean_text = self.clean_plate_text(plate_text)
    ...
    def publish_alerts(self, anomalies):
        ...
        try:
            ...
            for anomaly in anomalies:
                ...
                # 输出调试信息
                print(f"🚨 发布警报: {anomaly_type} (置信度: {anomaly['confidence']:.3f})")
    ...
    def process_plate_ocr_async(self, crop_img):
        """使用Tesseract-OCR的异步车牌OCR处理 - 增强版"""
        try:
            # 检查图像尺寸 - 放宽限制
            if crop_img.shape[0] < 15 or crop_img.shape[1] < 40:
                print(f"🔍 车牌图像太小: {crop_img.shape}")
                return "size_error", 0.0
            
            print(f"🔍 处理车牌图像: {crop_img.shape}")
            
            # 尝试多种预处理方法
            processed_images = self.preprocess_plate_multiple_methods(crop_img)
            ...
            for i, processed_img in enumerate(processed_images):
                for j, config in enumerate(self.tesseract_configs):
                    try:
                        # 方法1: 使用image_to_string直接识别
                        text_result = pytesseract.image_to_string(processed_img, config=config, lang='chi_sim+eng').strip()
                        if text_result:
                            # 清理识别结果
                            cleaned_text = self.clean_plate_text(text_result)
                            if len(cleaned_text) >= 4:
                                print(f"🎯 方法1成功 (预处理{i+1}, 配置{j+1}): '{text_result}' -> '{cleaned_text}'")
                                return cleaned_text, 0.8
                        
                        # 方法2: 使用image_to_data获取详细信息
                        ...
                        for k in range(len(data['text'])):
                            ...
                            conf = int(data['conf'][k]) if data['conf'][k] != '-1' else 0
                            
                            # 更宽松的过滤条件
                            if text and len(text) > 0 and conf > 10:  # 降低置信度阈值
                        ...
                        if valid_texts:
                            ...
                            cleaned_combined = self.clean_plate_text(combined_text)
                            
                            if len(cleaned_combined) >= 4:
                                avg_conf = sum(valid_confidences) / len(valid_confidences) / 100.0
                                
                                if avg_conf > best_confidence:
                                    best_result = cleaned_combined
                                    best_confidence = avg_conf
                                    print(f"🎯 方法2更新最佳结果 (预处理{i+1}, 配置{j+1}): '{combined_text}' -> '{cleaned_combined}' (置信度: {avg_conf:.3f})")
                        
                    except Exception as ocr_e:
                        print(f"⚠️ OCR尝试失败 (预处理{i+1}, 配置{j+1}): {ocr_e}")
                        continue
            
            # 返回最佳结果
            if best_result and best_confidence > 0.15:  # 降低最终置信度阈值
                print(f"✅ 最终OCR结果: '{best_result}' (置信度: {best_confidence:.3f})")
                return best_result, best_confidence
            elif best_result:
                print(f"⚠️ OCR结果置信度较低: '{best_result}' (置信度: {best_confidence:.3f})")
    ...
    def preprocess_plate_multiple_methods(self, crop_img):
        """多种车牌预处理方法"""
        processed_images = []
        
        try:
            # 方法1: 原始的Tesseract优化预处理
            processed1 = self.preprocess_plate_image_for_tesseract(crop_img)
            ...
            _, binary4 = cv2.threshold(equalized, 127, 255, cv2.THRESH_BINARY)
            ...
            _, binary5 = cv2.threshold(sharpened, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
            processed_images.append(binary5)
            
            print(f"🔧 生成了 {len(processed_images)} 种预处理图像")
    ...
    def preprocess_plate_image_for_tesseract(self, crop_img):
        """为Tesseract优化的车牌图像预处理 - 增强版"""
        try:
            print(f"🔧 开始预处理车牌图像: {crop_img.shape}")
            ...
            # 调整图像大小 - 车牌OCR需要足够大的图像
            height, width = gray.shape
            target_height = 64  # 增加目标高度
            
            if height < target_height:
                scale = target_height / height
                new_width = int(width * scale)
                gray = cv2.resize(gray, (new_width, target_height), interpolation=cv2.INTER_CUBIC)
                print(f"🔧 图像放大: {width}x{height} -> {new_width}x{target_height}")
            elif height > target_height * 2:
                scale = target_height * 1.5 / height
                new_width = int(width * scale)
                gray = cv2.resize(gray, (new_width, int(target_height * 1.5)), interpolation=cv2.INTER_AREA)
                print(f"🔧 图像缩小: {width}x{height} -> {new_width}x{int(target_height * 1.5)}")
            ...
            morphed = cv2.morphologyEx(adaptive_binary, cv2.MORPH_CLOSE, kernel_morph)
            
            # 去除小噪点
            kernel_open = cv2.getStructuringElement(cv2.MORPH_RECT, (1, 1))
            cleaned = cv2.morphologyEx(morphed, cv2.MORPH_OPEN, kernel_open)
            
            print(f"✅ 预处理完成: {cleaned.shape}")
            return cleaned
...
def parse_args():
    ...
    parser.add_argument('--fire-model', type=str, default='/home/robot/ros2_ws/src/example_python/models/fire_smoke_best.pt', help='火焰烟雾模型路径')
    parser.add_argument('--plate-model', type=str, default='/home/robot/ros2_ws/src/example_python/models/plate_best.pt', help='车牌模型路径')
    parser.add_argument('--face-model', type=str, default='/home/robot/ros2_ws/src/example_python/models/yolov8n-face.pt', help='人脸模型路径')
