...
sudo ldconfig
```
**如果编译或运行依旧报错提示，无法找到 "libVanJeeLaser760Filter_linux.so" 还需要做如下操作**
需要查看动态库权限，可执行如下指令
```sh
...
(4) 将ROS2环境下的雷达packet消息定义，将vanjee_lidar_msg工程解压到刚刚新建的*src*文件夹内，与vanjee_lidar_sdk并列。
...
[连接在线雷达数据并发送点云到ROS](doc/howto/02_how_to_decode_online_lidar_CN.md)

[解析PCAP包并发送点云到ROS](doc/howto/04_how_to_decode_pcap_file_CN.md)

[切换点类型](doc/howto/01_how_to_change_point_type_CN.md) 