...
`POINT_TYPE`为`XYZIRTT`时，vanjee_lidar_sdk使用Vanjee自定义的点类型```PointXYZRTT```。
...
vanjee_lidar_sdk将基于`PointXYZIRTT`的点云，转换为ROS的PointCloud2消息，再发布出去。
