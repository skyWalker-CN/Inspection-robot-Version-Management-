...
class AsyncNavigator(BasicNavigator):
    ...
    任务配置通过 String 话题 /waypoint_tasks 接收，推荐使用 JSON 格式：
    {
      "1": [
        {"type": "wait",   "duration": 5},
        {"type": "spin",   "angle": 90},
        {"type": "cmd_vel", "linear_x": 0.3, "angular_z": -0.3, "duration": 3},
        {"type": "gimbal", "action": 1, "value": 90}
      ],
      "5": [{"type": "spin", "angle": 360}],
      "8": [{"type": "cmd_vel", "linear_x": 0.3, "angular_z": 0.0, "duration": 3}]
    }
    点号从 1 开始，对应“第几个路径点”。
    """
    ...
    def __init__(self):
        ...
        # key: 0-based 全局路径点索引
        # value: [(task_func, kwargs), ...] 按顺序执行
        self.waypoint_tasks = {}

        # 原地旋转动作客户端
        self.spin_client = ActionClient(self, Spin, 'spin')

        # 底盘速度发布（可根据实际机器人修改话题名）
        self.cmd_vel_pub = self.create_publisher(Twist, '/cmd_vel_1', 10)

        # 云台控制话题，与 new_hksdk.py 的 /controlCam 订阅对应
    ...
    def load_task_config(self, text):
        """解析 String 话题中的 JSON 任务配置，并覆盖旧配置。"""
        try:
        ...
        if not isinstance(data, dict):
            self.get_logger().error("任务配置必须是 JSON 对象，例如 {\"1\": [...]}")
            return
        ...
        for key, value in data.items():
            ...
            waypoint_index = point_one_based - 1  # 转为 0-based
    ...
    def _build_task_func(self, task_type, kwargs):
        """旧接口用：根据 task_type 字符串构造 (func, kwargs)"""
    ...
    def _parse_task_dict(self, task):
        """解析 JSON 中的单个任务 dict，返回 (func, kwargs) 或 None"""
    ...
    def _perform_gimbal_task(self, action=None, value=0.0, command=None, hold=0.0):
        """发送云台控制命令到 /controlCam（与 new_hksdk.py 格式一致）"""
        try:
            if command is not None:
    ...
    def follow_waypoints_async(self, waypoints):
        """在后台线程中执行路径跟随。waypoints 应为最终要执行的全部路径点。"""
        with self.task_lock:
    ...
    def _follow_waypoints_thread(self, waypoints):
        ...
        try:
            ...
            global_offset = 0  # 已执行完的路径点数量（用于全局索引）
            ...
            while current_waypoint_set and rclpy.ok():
                for local_index, pose in enumerate(current_waypoint_set):
                    ...
                    total_global_count = len(waypoints)  # 原始路径总点数（可能因缓存路径变化）

                    # 为当前点创建带时间戳的 PoseStamped
    ...
    def _restamp_pose(self, pose):
        """重新生成带当前时间戳的 PoseStamped，避免 Nav2 拒绝旧时间戳目标"""
